...
\begin{document}
    ...
    \section{Unit 4: Borrowing for Large Purchases}
        ...
        \subsection{Housing}
            \subsubsection{DECISION TO RENT}
                \begin{itemize}
                    \item You don't plan on living in the area for more than 5 years
                    \item You cannot afford a 20\% downpayment
                    \item You prefer the fixed expense of rent to the variability of home ownership
                \end{itemize}
            \subsubsection{HOMWOWNERSHIP}
                \begin{itemize}
                    \item Consider ownership when you have:
                        \begin{itemize}
                            \item Down Payment saved (20\% or more ideal)
                            \item Location/employment stability (5 to 7 years)
                            \item Adequate resources to sustain homeownership
                        \end{itemize}
                    \item Average age of first-time home buyer is 31
                    \item 80\% of people 55-64 are homeowners
                \end{itemize}
            \subsubsection{TYPES OF MORTGAGES}
                \begin{enumerate}
                    \item Fixed-Rate or Conventional Mortgage:
                        \begin{itemize}
                            \item Fixed rate, term and payment for mortgage (PI)
                            \item TI can (and most likely will) change!
                            \item Typically offered over 15 or 30 years
                        \end{itemize}
                    \item Adjustable-Rate Mortgage or ARM:
                        \begin{itemize}
                            \item Rate can fluctuate over loan term
                            \item Risk of interest rate change assumed by borrower (not lender), typically 1-3 pp lower than conventional mortgage
                            \item If you don't have paymnet flexibility, DON'T get this kind of mortgage
                            \item PMT caps sometimes offered - BE WARY! This can lead to negative amortization (PMT < interest due, principal balance increases)
                        \end{itemize}
                \end{enumerate}
            \subsubsection{FEDERAL HOUSING ADMINISTRATION (FHA)}
                \begin{itemize}
                    \item Loan issued by an FHA-approved lender
                    \item Only need 3.5\% Down
                    \item Requires 580 credit score
                        \begin{itemize}
                            \item 500-579 with 10\% down
                        \end{itemize}
                    \item Upfront Private Mortgage Insurance (1.7\%) + Annual PMI for either 11 years or the life of the loan,
                        depending on LTV and length of loan
                \end{itemize}
            \subsubsection{THE "GREAT RECESSION"}
                \begin{itemize}
                    \item What was one of the big causes of the "Great Recession"?
                        \begin{itemize}
                            \item Glass-Stegall Act (1933)
                            \item Gramm-Leach-Bliley (1999)
                            \item Sub-Prime Loans
                            \item Dodd-Frank (2010) (Partially repealed in 2018)
                        \end{itemize}
                    \item Has it been resolved?
                        \begin{itemize}
                            \item Predatory Lending Practices
                            \item No Doc/Low Doc Loans
                        \end{itemize}
                \end{itemize}
            \subsubsection{HOW MUCH CAN YOU AFFORD?}
                \begin{itemize}
                    \item How much house can your budget afford?
                        \begin{enumerate}
                            \item Find Loan Payment = Total Monthly Budget Available - Non-loan Expenses
                            \item Find Loan Amount = Solve TVM for PV, using loan payment from step 1 as PMT
                            \item Find Sales Price = Loan Amount + Down Payment
                        \end{enumerate}
                \end{itemize}
            \subsubsection{AFFORDABILITY RATIOS USED BY LENDERS}
                \begin{itemize}
                    \item \textbf{Front-End Ratio}: PITI = 28\% of monthly gross income
                    \item \textbf{Back-End Ratio}: PITI = 36\% of monthly gross income - ALL other debt
                    \item The LOWER PITI of the two is used by the lender
                \end{itemize}
            \subsubsection{HOW MUCH SHOULD YOU SAVE FOR A DOWN PAYMENT?}
                \begin{itemize}
                    \item Lenders (banks or mortgage brokers) generally require 80\% Loan to Value (LTV) Ratio (=20\% DP)
                    \item LTV = Loan Amount / Purchase Price of Home (i.e., the \% of your house that you OWE)
                    \item A 20\% down payment is the minimum to avoid private mortgage insurance (PMI)
                        \begin{itemize}
                            \item this is to protect the lender ONLY!
                            \item Usually range from 0.5\% to 1.0\% of loan value
                        \end{itemize}
                    \item If you get PMI, make sure you keep track and ask for it to be removed when you have an LTV of 80\%!
                \end{itemize}
